...
class STM32L4xxSerial : public codal::Serial
{
     ...
     */
    int attach(SerialInterruptType interruptType, void (STM32L4xxSerial::*fp)());
    ...
    virtual int send(uint8_t *buffer, int bufferLen, SerialMode mode = DEVICE_DEFAULT_SERIAL_MODE)
    {
        ...
        while(!complete)
        {
            ...
            if(mode == SerialMode::ASYNC || bytesWritten >= bufferLen)
                complete = true;
        }
    ...
    virtual int read(uint8_t *buffer, int bufferLen, SerialMode mode = DEVICE_DEFAULT_SERIAL_MODE)
    {
        ...
        if(mode == SerialMode::SYNC_SPINWAIT)
        {
    ...
    virtual ManagedString readUntil(ManagedString delimeters, SerialMode mode = DEVICE_DEFAULT_SERIAL_MODE)
    {
        ...
        //matching character.
        if(mode == SerialMode::SYNC_SLEEP && foundIndex == -1)
        {
    ...
    virtual int redirect(Pin& tx, Pin& rx)
    {
        ...
        attach(RxInterrupt, &STM32L4xxSerial::dataReceived);

        if(txBufferedSize() > 0)
            attach(TxInterrupt, &STM32L4xxSerial::dataWritten);
     ...
     */
    virtual int eventAfter(int len, SerialMode mode = SerialMode::ASYNC)
    {
        if(mode == SerialMode::SYNC_SPINWAIT)
            return DEVICE_INVALID_PARAMETER;
     ...
     * @note delimeters are matched on a per byte basis.
     */
    virtual int eventOn(ManagedString delimeters, SerialMode mode = SerialMode::ASYNC)
    {
        if(mode == SerialMode::SYNC_SPINWAIT)
            return DEVICE_INVALID_PARAMETER;
    ...
    {
        attach(callback((Serial *)NULL, &STM32L4xxSerial::dataReceived),(interruptType == SerialInterruptType::RxInterrupt) ? RxIrq : TxIrq);
        return DEVICE_OK;
    }
};
}
